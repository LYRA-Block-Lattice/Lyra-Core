...
namespace Neo.Network.P2P
{
    public abstract class Peer : UntypedActor
    {
        ...
        public int MaxConnectionsPerAddress { get; private set; } = 100;
        ...
        private class SetDualBind : Inet.SocketOptionV2
        {
        ...
        private void OnStart(ChannelsConfig config)
        {
            ...
            {
                tcp_manager.Tell(new Tcp.Bind(Self, config.Tcp, options: new Inet.SocketOption [] { new Inet.SO.ReuseAddress(true), new SetDualBind() }));
            }
            //if (ListenerWsPort > 0)
            //{
            //    var host = "*";
