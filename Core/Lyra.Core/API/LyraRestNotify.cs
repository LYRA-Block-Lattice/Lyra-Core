...
namespace Lyra.Core.API
{
    public class LyraRestNotify : INotifyAPI
    {
        ...
        public async Task BeginReceiveNotifyAsync(string AccountID, string Signature, Action<NotifySource, string> action, CancellationToken cancel)
        {
            await Task.Factory.StartNew(async () => {
                while(true)
                {
                    ...
                    try
                    {
                        ...
                        {
                            action(result.Source, result.ExtraInfo);
                        }
                    }
                    catch(Exception ex)
                    {
