...
public class Tester
{
	public static async Task TestProc(JsWallet tester, JsWallet richGuy, string networkId, CancellationTokenSource cancel)
	{
		...
		(dynamic stkresult, _) = wstk.CallRPC("Staking to a profiting account", "StakingInfo CreateStakingAccount(string accountId, string Name, string voteFor, int daysToStake, bool compoundMode)", "CreateStakingAccount", new string[] {wstk.AccountId, "Stk1", pftresult.result.pftid, "1000", "True"});
		await Task.Delay(2000);
