...
namespace Lyra.Node.API.Controllers
{
    ...
    public class OpenAccountWithImportController : Controller
    {
        [HttpPost]
        [ActionName("OpenAccountWithImport")]
        public async Task<AuthorizationAPIResult> OpenAccountWithImport([FromBody] OpenAccountWithImportBlock block)
        {
            try
            {
                ...
                var result = await RPC.Client.OpenAccountWithImport(block);
                return result;
            }
