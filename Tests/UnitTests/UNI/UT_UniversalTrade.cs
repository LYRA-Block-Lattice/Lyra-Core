...
{
    [TestClass]
    public class UT_UniversalTrade : XUniTestBase
    {
        ...
        public async Task<IUniTrade> ResolveDisputeOnDAO(bool success)
        {
            ...
            if(success)
                Assert.IsTrue(trdblk.UTStatus == UniTradeStatus.DisputeClosed, $"dispute not close properly");
            else
                Assert.IsTrue(trdblk.UTStatus == UniTradeStatus.Dispute, $"dispute should not be closed");

            return trade;
        }

        private async Task LyraCouncilHandleDispute(IUniTrade trade)
        {
            // guest complain again to raise the dispute level
        ...
        private async Task ReplyToResolution(IUniTrade trade, ODRResolution resolution, bool success)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<IUniTrade> ResolveGuestComplainByHost(bool accepted)
        {
            await Setup();
        ...
        private async Task GuestComplaintWithdraw(IUniTrade trade, ComplaintClaim claim)
        {
            var reply = new ComplaintReply
            {
                created = DateTime.UtcNow,
        ...
        private async Task<IVoting> DaoOwnerCreateAVote(IUniTrade trade, ODRResolution resolution)
        {
        ...
        private async Task<IVoting> TheLordCreateAVote(Wallet devLord, IUniTrade trade, ODRResolution resolution)
        {
            var title = $"Now let vote on case ID {Random.Shared.NextInt64()}";
            VotingSubject subject = new VotingSubject
            {
        ...
        // peer/dealer level
        private async Task<ComplaintClaim> GuestComplainLevel0(IUniTrade trade, APIResultCodes shouldRet)
        {
            var lsb = await client.GetLastServiceBlockAsync();
            var brief0 = await GetBrief(lsb.GetBlock().Hash, trade.AccountID);
            Assert.AreEqual(trade.AccountID, brief0.TradeId);
        ...
        // dao level
        private async Task<ComplaintClaim> GuestComplainLevel1(IUniTrade trade)
        {
            var lsb = await client.GetLastServiceBlockAsync();
            var brief0 = await GetBrief(lsb.GetBlock().Hash, trade.AccountID);
            Assert.AreEqual(trade.AccountID, brief0.TradeId);
            ...
            //var crdptret = await test2Wallet.UniTradeRaiseDisputeAsync(trade.AccountID);
        ...
        // lyra council level
        private async Task<ComplaintClaim> GuestComplainLevel2(IUniTrade trade)
        {
            var lsb = await client.GetLastServiceBlockAsync();
            var brief0 = await GetBrief(lsb.GetBlock().Hash, trade.AccountID);
            Assert.AreEqual(trade.AccountID, brief0.TradeId);
        ...
        public async Task ResolveDisputeOnDAO()
        {
            ...
            var trade = (await client.GetLastBlockAsync(tradeid)).As<IUniTrade>();
            Assert.IsTrue(trade != null && trade.UTStatus == UniTradeStatus.Dispute);
        ...
        private async Task<ODRResolution> CreateODRResolution(IUniTrade trade)
        {
            var lsb = await client.GetLastServiceBlockAsync();
            var brief00 = await GetBrief(lsb.GetBlock().Hash, trade.AccountID);
